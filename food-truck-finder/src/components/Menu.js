...
//row-start-1 col-start-2  name
    ...
    return (
        <div className='w-100 h-100'>
            {menuItems.map((item) => (
                <div key={item._id} className='h-50 p-4 grid overflow-hidden grid-cols-3 gap-x-3'>
                    <img src={food} alt='food'
                    className='rounded-lg w-26 md:w-30 box row-start-1 row-end-4 col-start-1 my-1'
                    />
                    <h3 className='h-6 text-sm font-bold py-0 col-start-2 col-span-2'>{item.name}<span className='ml-10'>{`$${item.price}`}</span></h3>
